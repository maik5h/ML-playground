...
class InteractiveGaussian(Gaussian, TrainableModel):
    """
    Extends the Gaussian class by methods to plot the distribution and
    update parameters based on matplotlib callbacks. A feature vector
    phi must be forwarded to objects of this class. The random
    variables forming this distribution are interpreted as weights
    being multiplied with this feature vector.
